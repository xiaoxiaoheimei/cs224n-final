...
class BiDAF(nn.Module):
    ...
    def forward(self, cc_idxs, qc_idxs):
        ...
        c_enc = self.enc(cc_emb, cc_len)    # (batch_size, c_len, 2 * hidden_size)
        q_enc = self.enc(qc_emb, qc_len)    # (batch_size, q_len, 2 * hidden_size)

        att = self.att(c_enc, q_enc,
                       cc_mask, qc_mask)    # (batch_size, c_len, 8 * hidden_size)

        mod = self.mod(att, cc_len)        # (batch_size, c_len, 2 * hidden_size)

        out = self.out(att, mod, cc_mask)  # 2 tensors, each (batch_size, c_len)
